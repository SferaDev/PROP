...
public class LoadView extends GridPane {
    ...
    private final Label noGamesLabel = createLabel(LocaleUtils.getInstance().getString("NO_GAMES"));
    ...
    private void populateNewGameBox(VBox newGameBox, RaisedButton button) {
        ...
        labelPegs.setFont(Font.font(14));
        ...
        labelColors.setFont(Font.font(14));
    ...
    private void populateSavedGameBox(VBox savedGameBox, RaisedButton button) {
        ...
        noGamesLabel.prefHeightProperty().bind(savedGameBox.heightProperty());
        ...
        button.setOnAction(event -> {
            ...
            if (game != null) PresentationController.getInstance().requestStartSavedGame(game);
        });
        savedGameBox.getChildren().add(gameListView);
        savedGameBox.getChildren().add(noGamesLabel);
    ...
    public void reset() {
        ...
        gameListView.setVisible(gameListView.getItems().size() != 0);
        noGamesLabel.setVisible(gameListView.getItems().size() == 0);
        gameListView.setManaged(gameListView.getItems().size() != 0);
        noGamesLabel.setManaged(gameListView.getItems().size() == 0);
    }
}
